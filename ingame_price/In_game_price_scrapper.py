    ...
    def find_and_click_image(image_path, folder_dir, map_name, scale_range=np.linspace(0.25, 2, 8)): 
        ...
        for scale in scale_range:
            ...
            if max_val >= 0.85: # Threshold for a match
                ...
                if image_path != rf"{folder_dir}STOP.jpg" and image_path != rf"{folder_dir}HDV_ITEM.jpg":
                    move_with_jitter(start_pos, end_pos, steps=2) # Move to the found position
                    time.sleep(random.uniform(0, 0.1))
                    left_click()   # Click the image
                ...
                elif image_path == rf"{folder_dir}HDV_ITEM.jpg":
                    print("HDV Found")
                    input("Click to open the HDV and enter continue")
                    left_click(600, 580)
                    left_click(645, 580)
