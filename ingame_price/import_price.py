            ...
            for row in reader:
                ...
                try:
                    ...
                    try:
                        ...
                        print(f"Multiple items found with name: {item_name}")
                    ...
                    try:
                        ...
                        for db_item in all_items:
                            sanitized_db_name = sanitize_filename(db_item.name).title()
                        
                            if sanitized_db_name == item_name:
                        ...
                        if matched_item:
                            ...
                            potential_items = Item.objects.filter(name__icontains=item_name.split('_')[0] if '_' in item_name else item_name)
                            ...
                            if potential_items.exists():
                                ...
                                for potential_item in potential_items:
                                    sanitized_db_name = sanitize_filename(potential_item.name).title()
                                    # Compare the CSV name with the sanitized DB name
                                    similarity = sum(c1 == c2 for c1, c2 in zip(item_name, sanitized_db_name))
                        ...
                        print(f"Error finding item {item_name}: {str(e)}")
                        item_found = False
                
                # If item wasn't found, add to missing items and continue
                if not item_found:
                    print(f"Item not found: {item_name}")
                    missing_writer.writerow([item_name, price_value])
                    missing_items += 1
                    continue
                ...
                try:
                    ...
                    print(f"Error updating price for {item_name}: {str(e)}")
